...
fn expr_to_syn(module: &Module, expr: &IrExpr) -> Expr {
    match expr {
            ...
            _ => Expr::Struct(ExprStruct {
                ...
                dot2_token: {
                    let structure = module
                        ...
                        .unwrap_or_else(|| panic!("No struct for {structure:#?}"));
        ...
        | IrExpr::Gt(lhs, rhs)
        | IrExpr::And(lhs, rhs)
        | IrExpr::Or(lhs, rhs) => Expr::Paren(syn::ExprParen {
            attrs: vec![],
            paren_token: Default::default(),
            expr: Box::new(Expr::Binary(ExprBinary {
                ...
                op: match expr {
                    ...
                    IrExpr::Eq(_, _) => BinOp::Eq(Default::default()),
                    IrExpr::Ne(_, _) => BinOp::Ne(Default::default()),
                    IrExpr::Lt(_, _) => BinOp::Lt(Default::default()),
                    IrExpr::Gt(_, _) => BinOp::Gt(Default::default()),
                    IrExpr::And(_, _) => BinOp::And(Default::default()),
                    IrExpr::Or(_, _) => BinOp::Or(Default::default()),
                    _ => unreachable!(),
                },
        ...
        | IrExpr::Max(lhs, rhs)
        | IrExpr::Dot(lhs, rhs)
        | IrExpr::Atan2(lhs, rhs) => Expr::MethodCall(ExprMethodCall {
            attrs: vec![],
            receiver: Box::new(expr_to_syn(module, lhs)),
            dot_token: Default::default(),
            method: match expr {
                ...
                IrExpr::Atan2(_, _) => Ident::new("atan2", Span::call_site()),
                _ => unreachable!(),
            },
            turbofish: None,
            paren_token: Default::default(),
